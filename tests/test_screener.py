...
def test_ai_screener():
    ...
    print(
        f"Stop Loss: "
        f"{top_pick['Stop_Loss']}"
    )

    print(
        f"Take Profit: "
        f"{top_pick['Take_Profit']}"
    )

    print(
        f"Risk Reward: "
        f"{top_pick['Risk_Reward']}"
    )
...
def test_score_distribution():
    ...
    max_score = (
        screener_df["Score"].max()
    )

    min_score = (
        screener_df["Score"].min()
    )
...
def test_risk_reward():
    ...
    if screener_df.empty:
        ...
        print(
            "⚠️ No screener result"
        )

        return

    strong_rr = screener_df[
        screener_df["Risk_Reward"] >= 2
    ]
